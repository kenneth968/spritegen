...
def test_project_generator_passes_prior_generated_images_as_openai_references(
    tmp_path,
    monkeypatch,
):
    from PIL import Image
...
def test_project_generator_prefers_selected_export_variant_as_reference(tmp_path, monkeypatch):
    from PIL import Image
    ...
    previous_dir = store.generated_dir(project.slug) / existing.slug
    previous_dir.mkdir(parents=True)
    previous_raw_1 = previous_dir / "single-v01.png"
    previous_raw_2 = previous_dir / "single-v02.png"
    Image.new("RGBA", (1024, 1024), (255, 0, 0, 255)).save(previous_raw_1)
    Image.new("RGBA", (1024, 1024), (0, 255, 0, 255)).save(previous_raw_2)
    (previous_dir / "generation_manifest.json").write_text(
        json.dumps(
            {
                "outputs": [
    ...
    export_dir = store.project_dir(project.slug) / "exports" / existing.slug
    ...
    assert seen["reference_images"] == [previous_raw_2]
    manifest = json.loads(result.manifest_path.read_text(encoding="utf-8"))
    assert manifest["reference_images"] == [str(previous_raw_2)]
